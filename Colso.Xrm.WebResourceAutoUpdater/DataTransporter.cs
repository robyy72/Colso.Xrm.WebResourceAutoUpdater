                        ...
                        bwFill.DoWork += (sender, e) =>
                        {
                            ...
                            foreach (AttributeMetadata attribute in attributes)
                            {
                                ...
                                if (typename != "EntityNameType")
                                {
                                    ...
                                    // Only check values which ar not previously unmarked and that we can update
                                    if (!unmarkedattributes.Contains(attribute.LogicalName) 
                                    && ((attribute.IsValidForUpdate != null && attribute.IsValidForUpdate.Value) || (typename != "UniqueIdentifierType")))
                                        item.Checked = true; 
        ...
        private void TransferEntities(bool preview)
        {
            ...
            if (!preview)
            {
                foreach (ListViewItem entityitem in lvEntities.SelectedItems)
                {
                    if (entityitem != null && entityitem.Tag != null)
                    {
                        var primaryIdAttribute = ((Microsoft.Xrm.Sdk.Metadata.EntityMetadata)entityitem.Tag).PrimaryIdAttribute;
                        if (!lvAttributes.CheckedItems.Cast<ListViewItem>().Select(v => (AttributeMetadata)v.Tag).Where(a => a.LogicalName == primaryIdAttribute).Any()) { 
                            var msg = string.Format("The primary id attribute \"{0}\" is not selected.", primaryIdAttribute);
                            if (cbCreate.Checked) msg += " All the source records will be created and it won't be possible to match the two records by the unique identifier.";
                            if (cbUpdate.Checked) msg += " No target records will be updated.";
